...
def convert_padding_mask_to_attention_mask(sequence, padding_mask):
    ...
    """
    batch_assert = tf.assert_equal(tf.shape(padding_mask)[0], tf.shape(sequence)[0],message='batch size mismatch between input sequence and padding_mask')
    rank_assert = tf.assert_equal(tf.rank(padding_mask), 2, message='Can only convert 2D position mask to 3D attention mask')
...
def convert_sequence_length_to_sequence_mask(sequence, sequence_lengths):
    ...
    """
    batch_assert = tf.assert_equal(tf.shape(sequence_lengths)[0], tf.shape(sequence)[0], message='batch size mismatch between input sequence and sequence_lengths')
    rank_assert = tf.assert_equal(tf.rank(sequence_lengths), 1, message='Can only convert 1D sequence_lengths to 2D mask')
