...
class TransformerEncoderBlock(Model):
    ...
    def call(self, inputs, self_attention_mask=None):
        ...
        q=norm_inputs
        m=norm_inputs
        attn = self.self_attention((q,m),mask=self_attention_mask)
...
class TransformerDecoderBlock(Model):
    ...
    def call(self, decoder_inputs, encoder_outputs, self_attention_mask=None, cross_attention_mask=None):    
        ...
        q=norm_decoder_inputs
        m=norm_decoder_inputs
        target_selfattn = self.self_attention((q,m),mask=self_attention_mask)
        ...
        q=norm_target_selfattn
        m=norm_encoder_outputs
        encdec_attention = self.cross_attention((q,m),mask=cross_attention_mask)
...
class Transformer(Model):
    ...
    def call(self, source_sequence, target_sequence, encoder_mask, decoder_mask, mask_future=True, shift_target_sequence_right=True):
        ...
        # Using the self.decoder, we provide it with the decoder input, and the encoder_output. 

        # As usual, we provide it with the encoder and decoder_masks
        # Finally, we also pass it these two optional arguments:
