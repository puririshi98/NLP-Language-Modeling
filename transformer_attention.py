...
class AttentionQKV(Model):
    ...
    def call(self, queries, keys, values, mask=None):
        ...
        similarity = tf.matmul(queries,keys,transpose_b=True)/(tf.sqrt(tf.cast(key_dim,tf.float32)))
...
class MultiHeadProjection(Model):
    ...
    def _split_heads(self, tensor):
        ...
        tensor = tf.reshape(tensor,(tf.cast(batch_size,tf.int32),tf.cast(tensorlen,tf.int32),tf.cast(self.n_heads,tf.int32),tf.cast(new_feature_size,tf.int32)))

        # Transpose the matrix so the outer-dimensions are the batch-size and the number of heads
        tensor = tf.transpose(tensor,perm=[0,2,1,3])
        #tf.concat(tf.split(tensor, self.n_heads, axis=2), axis=0)
        return tensor
        ##########################################################################################
    ...
    def _combine_heads(self, tensor):
        ...
        new_feature_size = self.n_heads*feature_size #the new feature size, if we combine all the heads
        tensor = tf.reshape(tensor,(tf.cast(batch_size,tf.int32),tf.cast(tensorlen,tf.int32),tf.cast(new_feature_size,tf.int32))) # Reshape the Tensor to remove the heads dimension and come back to a Rank-3 tensor
